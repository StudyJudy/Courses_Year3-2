...
protoboard<FieldT> build_protoboard(int *secret)
{
    ...
    // 定义所有需要外部输入的变量以及中间变量
    pb_variable<FieldT> x;
    pb_variable<FieldT> temp_x2;
    pb_variable<FieldT> temp_x3;
    pb_variable<FieldT> temp_addx;
    pb_variable<FieldT> out;
    ...
    out.allocate(pb, "out");
    x.allocate(pb, "x");
    temp_x2.allocate(pb, "temp_x2");
    temp_x3.allocate(pb, "temp_x3");
    temp_addx.allocate(pb, "temp_addx");
    // 定义公有的变量的数量，set_input_sizes(n)用来声明与 protoboard 连接的 public 变量的个数 n。
    // 在这里 n = 1，表明与 pb 连接的前 n = 1 个变量是 public 的，其余都是 private 的。
    // 因此，要将 public 的变量先与 pb 连接（前面 out 是公开的）。
    pb.set_input_sizes(1);
    // 为公有变量赋值
    // pb.val(out) = 0;
    pb.val(out) = 35;
    // 至此，所有变量都已经顺利与 protoboard 相连，下面需要确定的是这些变量间的约束关系。
    // 如下调用protoboard 的add_r1cs_constraint()函数，为pb添加形如a * b = c的r1cs_constraint。即r1cs_constraint<FieldT>(a, b, c)中参数应该满足a * b = c。
    // 根据注释不难理解每个等式和约束之间的关系。
	// x-1= w_1
    ...
    // 根据注释不难理解每个等式和约束之间的关系。
    // x*x = temp_x2
    pb.add_r1cs_constraint(r1cs_constraint<FieldT>(x, x, temp_x2));
    // temp_x2 * x = temp_x3
    pb.add_r1cs_constraint(r1cs_constraint<FieldT>(temp_x2, x, temp_x3));
    // temp_x3 + x = temp_addx
    pb.add_r1cs_constraint(r1cs_constraint<FieldT>(temp_x3 + x, 1, temp_addx));
    // temp_addx + 5 = out
    pb.add_r1cs_constraint(r1cs_constraint<FieldT>(temp_addx + 5, 1, out));
    ...
    {
		pb.val(x) = secret[0];
		pb.val(temp_x2) = secret[1];
		pb.val(temp_x3) = secret[2];
		pb.val(temp_addx) = secret[3];
    }
    return pb;
}
