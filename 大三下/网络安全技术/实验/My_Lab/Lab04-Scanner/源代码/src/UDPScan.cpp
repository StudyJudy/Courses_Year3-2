...
void UDPScanHost(struct UDPScanHostThrParam* p)
{
	...
	do
	{
		...
		if (n > 0)
		{
			...
			// type字段的值是否是3
			// cout << "hdr.ip.saddr: " << hdr.ip.saddr << endl;
			// cout << "inet_addr(&HostIP[0]): " << inet_addr(&HostIP[0]) << endl;
			// cout << "hdr.icmp.code: " << hdr.icmp.code << endl;
			// cout << "hdr.icmp.type: " << hdr.icmp.type << endl;
